...
:: Iniciar Backend usando VBScript para ocultar la ventana
echo Set WshShell = CreateObject("WScript.Shell") > "%temp%\start_backend.vbs"
echo WshShell.Run "cmd /c ""cd /d %BASE_DIR%\server && venv\Scripts\activate && uvicorn main:app --port 8000""", 0, false >> "%temp%\start_backend.vbs"
wscript.exe "%temp%\start_backend.vbs"
